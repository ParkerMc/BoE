...
func (mongo *Mongo) UserAddRole(userID string, role string) *model.Error {
	user, detailErr := mongo.UserGetByID(userID)
...
// UserGetByEmail returns user with given email
func (mongo *Mongo) UserGetByEmail(email string) (*model.User, *model.Error) {
...
// UserGetByID returns user with given id
func (mongo *Mongo) UserGetByID(id string) (*model.User, *model.Error) {
...
// UserGetByUsername returns user with given username
func (mongo *Mongo) UserGetByUsername(username string) (*model.User, *model.Error) {
	for i := range mongo.userCache { // Loop through the cache
...
// UserGetUsers gets users GetBy prams
...
func (mongo *Mongo) UserLogin(userID string, password string, ip string) (*model.User, *model.UserToken, *model.Error) {
	...
	if strings.Contains(userID, "@") { // If there userID contains a @ then get user by email
		user, detailErr = mongo.UserGetByEmail(userID)
	} else { // Else look for a user by username
		user, detailErr = mongo.UserGetByUsername(userID)
	}
	if detailErr != nil { // If there is an error return it
...
func (mongo *Mongo) UserRegister(username string, name string, password string, email string) *model.Error {
	_, detailErr := mongo.UserGetByUsername(username)
	if detailErr != nil {
		if detailErr.Type != model.ErrorNotFound { // If there is a database error return with it
			return detailErr
		}
	} else { // If there is no error then user was found return saying username was taken
		return model.NewError("Username taken.", model.ErrorConflict)
	}
	_, detailErr = mongo.UserGetByEmail(email)
	if detailErr != nil {
		if detailErr.Type != model.ErrorNotFound { // If there is a database error return with it
			return detailErr
		}
	} else { // If there is no error then user was found return saying email was taken
		return model.NewError("Email taken.", model.ErrorConflict)
	}
